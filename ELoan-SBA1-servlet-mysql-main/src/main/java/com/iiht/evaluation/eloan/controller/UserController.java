...
private ConnectionDao connDao;
	...
	/* write the code to place the loan information */
		...
		Loaninfo l = new Loaninfo;
		 ...
		 l.applno=getParameter(applno);
		 l.purpose=getParameter(purpose);
		 l.amtrequest=getParameter(amtrequest);
		 l.doa=java.time.LocalDate.now();
		 l.bstructure=getParameter(bstructure);
		 l.bindicator=getParameter(bindicator);
		 l.address=getParameter(address);
	...
	private String editLoanProcess(HttpServletRequest request, HttpServletResponse response) throws SQLException {
		...
		Loaninfo l1 = new Loaninfo;
		
		 l1.applno=getParameter(applno);
		 l1.purpose=getParameter(purpose);
		 l1.amtrequest=getParameter(amtrequest);
		 l1.doa=java.time.LocalDate.now();
		 l1.bstructure=getParameter(bstructure);
		 l1.bindicator=getParameter(bindicator);
		 l1.address=getParameter(address);
	...
	private String registerUser(HttpServletRequest request, HttpServletResponse response) throws SQLException {
		...
		response.sendRedirect(request.getContextPath() + "/newuserui.jsp")
	...
	private String displaystatus(HttpServletRequest request, HttpServletResponse response) throws SQLException {
		...
		*/
		...
		int applno=request.geParameter(applno);
	...
	/* write a code to return to editloan page */
		...
		response.sendRedirect(request.getContextPath() + "/editloan.jsp")
	...
	/* write a code to return to trackloan page */
		response.sendRedirect(request.getContextPath() + "/trackloan.jsp")
